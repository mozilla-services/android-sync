...
package org.mozilla.android.sync.config.test;
...
public class TestConfigurationMigrator extends AndroidSyncTestCase {
  ...
  public void testMigrate0to1() throws Exception {
    ...
    Account account = null;
    try {
      account = SyncAccounts.createSyncAccount(syncAccount); // Wipes prefs.
    ...
    try {
      ...
      assertEquals(new Long(TEST_NUM_CLIENTS).toString(), accountManager.getUserData(account, V0_PREF_NUM_CLIENTS));
    } finally {
      if (account != null) {
        TestSyncAccounts.deleteAccount(this, accountManager, account);
      }

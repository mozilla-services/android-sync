 ...
 * accounts should be disabled. (Bug 714304.)
 *
 ...
 *
 */
public class TestUpgradeRequired extends MockAccountTest {
  /**
   * Verify that when SyncAdapter disables the account it's syncing when
   * informed of an Upgrade Required response.
   */
  public void testInformUpgradeRequired() {
    final AccountManager accountManager = AccountManager.get(context);
    final Account account = UpgradeRequiredHelper.getTestAccount(accountManager, TEST_USERNAME);
    ...
    assertFalse(UpgradeRequiredHelper.syncsAutomatically(account));
    assertTrue(UpgradeRequiredHelper.isSyncable(account));
    assertTrue(UpgradeRequiredHelper.willEnableOnUpgrade(account, accountManager));
  }

...
public class TestReadingListClient {
  ...
  public final void test() throws Exception {
    final AuthHeaderProvider auth = new BasicAuthHeaderProvider("test_android_client_3", "nopassword");
    ...
    Assert.assertEquals(200, sinceDelegate.response.getStatusCode());
    Assert.assertEquals(1, sinceDelegate.records.size());

    // The response should have a Last-Modified.
    final long lastModified = sinceDelegate.response.getLastModified();
    Assert.assertTrue(lastModified > 1425428783535L);

    // Another fetch with an If-Modified-Since should return a 304.
    final TestRecordDelegate nothingDelegate = fetchIMS(client, lastModified);
    ...
    Assert.assertEquals(200, afterDelegate.response.getStatusCode());
  ...
  public final void testWithAuthorization() throws Throwable {
    ...
    final ClientReadingListRecord record = new ClientReadingListRecord("http://reddit.com", "Reddit", "Test Device");

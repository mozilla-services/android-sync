...
public class TestFreshStart {
  ...
  public void testLiveFreshStart() throws Exception {
    ...
    String jsonInfoCollections = TestBasicFetch.realLiveFetch(TEST_USERNAME, TEST_PASSWORD, TEST_SYNC_KEY, session.config.infoURL(), false);
    InfoCollections infoCollections = new InfoCollections(null, null);
    ExtendedJSONObject o = ExtendedJSONObject.parseJSONObject(jsonInfoCollections);
    ...
    String jsonMetaGlobal = TestBasicFetch.realLiveFetch(TEST_USERNAME, TEST_PASSWORD, TEST_SYNC_KEY, session.config.metaURL(), false);
    assertNotNull(jsonMetaGlobal);
    MetaGlobal mg = new MetaGlobal(null, null);
    mg.setFromRecord(CryptoRecord.fromJSONRecord(jsonMetaGlobal));
    ...
    String jsonCryptoKeys = TestBasicFetch.realLiveFetch(TEST_USERNAME, TEST_PASSWORD, TEST_SYNC_KEY, session.config.keysURI().toString(), false);

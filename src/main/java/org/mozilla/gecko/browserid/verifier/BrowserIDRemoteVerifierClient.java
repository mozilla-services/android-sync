...
public class BrowserIDRemoteVerifierClient implements BrowserIDVerifierClient {
  ...
  public void verify(String audience, String assertion, final BrowserIDVerifierDelegate delegate) {
    ...
    r.delegate = new BaseResourceDelegate(r) {
      ...
      public void handleHttpResponse(HttpResponse response) {
        ...
        if (statusCode != 200) {
          delegate.handleError(new Exception("Expected status code 200."));
          return;
        }

        if ("failure".equals(o.getString("status"))) {
          delegate.handleFailure(o.getString("reason"));
          return;
        }

        if (!("okay".equals(o.getString("status")))) {
          delegate.handleError(new BrowserIDVerifierMalformedResponseException("Expected status = 'okay'."));
          return;
        }
  ...
  public static class BrowserIDVerifierMalformedResponseException extends Exception {

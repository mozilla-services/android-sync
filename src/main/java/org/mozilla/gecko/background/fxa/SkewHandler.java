...
public class SkewHandler {
  ...
  public SkewHandler(final String url) {
    this.url = url;
  }

  public boolean updateSkewFromServerMillis(long millis, long now) {
    skewMillis = millis - now;
    Logger.debug(LOG_TAG, "Updated skew: " + skewMillis + "ms for URL " + this.url);
    return true;
  }
  ...
  public boolean updateSkewFromHTTPDateString(String date, long now) {
    try {
      ...
      Logger.warn(LOG_TAG, "Unexpected: invalid Date header from " + this.url);
      return false;
    }
  }
  ...
  public boolean updateSkewFromDateHeader(Header header, long now) {
    ...
    if (null == date) {
      Logger.warn(LOG_TAG, "Unexpected: null Date header from " + this.url);
      return false;
    }
  ...
  public boolean updateSkew(HttpResponse response, long now) {
    ...
    if (null == header) {
      Logger.warn(LOG_TAG, "Unexpected: missing Date header from " + this.url);
      return false;
    }

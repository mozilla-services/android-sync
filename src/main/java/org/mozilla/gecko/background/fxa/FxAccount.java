...
public class FxAccount {
  ...
  public static FxAccount makeFxAccount(String email, byte[] stretchedPWBytes, String mainSalt, String srpSalt)
  ...
  public SRPSession srpSession(String srpB, BigInteger a)
    ...
    byte[] kBytes = Utils.sha256(Utils.concatAll(
        Utils.hex2Byte(N.toString(16), params.byteLength),
        Utils.hex2Byte(g.toString(16), params.byteLength)));
    ...
    String srpS = params.hexModN(S);

    // M = H(pad(A) | pad(B) | pad(S))
    byte[] Mbytes = Utils.sha256(Utils.concatAll(
        Utils.hex2Byte(srpA),
        Utils.hex2Byte(srpB),
        Utils.hex2Byte(srpS)));

    // K = H(pad(S))
    byte[] Kbytes = Utils.sha256(Utils.hex2Byte(srpS));

    return new SRPSession(N, a, A, S, Mbytes, Kbytes);
  }

  public SRPSession srpSession(String srpB)
      throws UnsupportedEncodingException, GeneralSecurityException {
    // XXX We should take care to get security to review how we generate a.
    BigInteger a = Utils.generateBigIntegerLessThan(params.N);

...
public class PICLServer0RepositorySession extends RepositorySession {
  ...
  public static final String LOG_TAG = PICLServer0RepositorySession.class.getSimpleName();
  ...
  public void fetchSince(long timestamp, final RepositorySessionFetchRecordsDelegate delegate) {
    ...
    delegateQueue.execute(new Runnable() {
      @Override
      public void run() {
        Logger.warn(LOG_TAG, "Not fetching any records.");
  ...
  //  }
  //
  //  public static final String LOG_TAG = "PICL0Session";
  //
  //  private static final int UPLOAD_BYTE_THRESHOLD = 1024 * 1024;    // 1MB.
  //  private static final int UPLOAD_ITEM_THRESHOLD = 100;
  ...
  //
  //
  //  PICLServer0Repository serverRepository;
  ...
  //    // Right now, we only work with TabsRecords. So.. don't die.
  //    TabsRecord tabsRecord = (TabsRecord) record;
  //
  //    StringBuilder jsonBuilder = new StringBuilder();
  //    jsonBuilder.append("{\"id\":").append(tabsRecord.guid).append(",\"payload\":[");
  //
  //    for (Tab tab : tabsRecord.tabs) {
  //      jsonBuilder.append(TabsRecord.tabToJSONObject(tab));
  //    }
  //
  //    jsonBuilder.append("]}");
  //
  ...
  //    try {
  //      json = jsonBuilder.toString().getBytes("UTF-8");
  //    } catch (UnsupportedEncodingException e) {
  //      //if utf-8 is unsupported, what is life?
  //    }
  //
  //
  ...
  //      PICLStorageRequest request = new PICLStorageRequest(u);
  //

...
public class ReadingListSynchronizer {
  ...
  private static class ModifiedUploadDelegate implements ReadingListRecordUploadDelegate {
    ...
    public void onFailure(ClientReadingListRecord up, MozResponse response) {
      ...
      if (response.getStatusCode() == 404) {
        ...
        Logger.warn(LOG_TAG, "Ignoring 409 response patching record with guid: " + up.getGUID());
      } else {
        // We should never see a 412 since we race to upload our changes (and accept whatever the server gives us back).
        recordFailed(up);
      }
    }
  ...
  private Queue<ClientReadingListRecord> collectModifiedFromCursor(final Cursor cursor) {
    try {
      ...
      while (cursor.moveToNext()) {
        ...
        final ExtendedJSONObject o = new ExtendedJSONObject();
        o.put("id", guid);
        o.put("excerpt", cursor.getString(columnExcerpt)); // Can be NULL.
        o.put("resolved_url", cursor.getString(columnResolvedURL)); // Can be NULL.
        o.put("resolved_title", cursor.getString(columnResolvedTitle)); // Can be NULL.
  ...
  protected void uploadModified(final StageDelegate delegate) {
    try {
      ...
      // uploaded and removed from local storage. So what's left are material
      // changes.  In any case, we only collect the material changes.

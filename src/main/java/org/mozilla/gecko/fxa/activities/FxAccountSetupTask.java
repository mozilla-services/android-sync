...
abstract class FxAccountSetupTask<T> extends AsyncTask<Void, Void, InnerRequestDelegate<T>> {
  ...
  public FxAccountSetupTask(Context context, String email, String password, FxAccountClient20 client, RequestDelegate<T> delegate) throws UnsupportedEncodingException {
  ...
  public static class FxAccountSignUpTask extends FxAccountSetupTask<String> {
    ...
    public FxAccountSignUpTask(Context context, String email, String password, FxAccountClient20 client, RequestDelegate<String> delegate) throws UnsupportedEncodingException {
    ...
    protected InnerRequestDelegate<String> doInBackground(Void... arg0) {
      try {
        client.createAccount(emailUTF8, passwordUTF8, false, innerDelegate);
        latch.await();
  ...
  public static class FxAccountSignInTask extends FxAccountSetupTask<LoginResponse> {
    ...
    protected InnerRequestDelegate<LoginResponse> doInBackground(Void... arg0) {
      try {
        client.loginAndGetKeys(emailUTF8, passwordUTF8, innerDelegate);
        latch.await();
        return innerDelegate;
      } catch (InterruptedException e) {

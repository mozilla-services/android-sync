...
public class FxAccountDeletedService extends IntentService {
  ...
  protected void onHandleIntent(final Intent intent) {
    ...
    final FxAccountOAuthClient10 oauthClient = new FxAccountOAuthClient10(oauthServerURI, new Executor() {
        @Override
    ...
    for (String token : tokens) {
      oauthClient.deleteToken(token, new FxAccountAbstractClient.RequestDelegate<Void>() {
          ...
          public void handleError(Exception e) {
            Logger.error(LOG_TAG, "Failed to delete cached OAuth token.", e);
          }

          @Override
          public void handleFailure(FxAccountAbstractClientRemoteException e) {
            Logger.error(LOG_TAG, "Exception during cached OAuth token deletion.", e);
          }
      });
    }
  }

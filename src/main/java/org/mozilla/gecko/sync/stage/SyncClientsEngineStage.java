...
public class SyncClientsEngineStage implements GlobalSyncStage {
  ...
  public class ClientDownloadDelegate extends WBOCollectionRequestDelegate {
    ...
    public void handleRequestSuccess(SyncStorageResponse response) {
      ...
      session.config.persistServerClientsTimestamp(response.normalizedTimestamp());
  ...
  public class ClientUploadDelegate extends WBORequestDelegate {
    ...
    public void handleRequestSuccess(SyncStorageResponse response) {
      ...
      try {
        ...
        long timestamp = response.normalizedTimestamp();
        session.config.persistServerClientRecordTimestamp(timestamp);

        if (timestamp > session.config.getPersistedServerClientsTimestamp()) {
          session.config.persistServerClientsTimestamp(timestamp);
        }
        BaseResource.consumeEntity(response);

        Logger.debug(LOG_TAG, "Header timestamp is " + response.normalizedWeaveTimestamp());
        Logger.debug(LOG_TAG, "Body is: " + response.body());
        Logger.debug(LOG_TAG, "Timestamp from header or body is: " + timestamp);
      } catch (Exception e) {

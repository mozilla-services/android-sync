...
public class ConfigurationMigrator {
  ...
  protected static int upgradeAndroidAccount0to1(final AccountManager accountManager, final Account account, final SharedPreferences to) throws Exception {
    ...
    if (accountGUID != null) {
      ...
      editor.putString(V1_PREF_ACCOUNT_GUID, accountGUID);
      count += 1;
    }
    if (clientName != null) {
      Logger.debug(LOG_TAG, "Migrated client name.");
      editor.putString(V1_PREF_CLIENT_NAME, clientName);
      count += 1;
    }
    if (numClients > -1) {
      Logger.debug(LOG_TAG, "Migrated clients count.");
      editor.putLong(V1_PREF_NUM_CLIENTS, numClients);
      count += 1;
    }
  ...
  public static void ensurePrefsAreVersion(final long desiredVersion,
      ...
      final String product, final String username, final String serverURL, final String profile) throws Exception {
    if (desiredVersion < 0 || desiredVersion > SyncConfiguration.CURRENT_VERSION) {
    ...
    long currentVersion = versionPrefs.getLong(SyncConfiguration.PREF_VERSION, 0);
    if (currentVersion == desiredVersion) {
      Logger.info(LOG_TAG, "Current version (" + currentVersion + ") is desired version; no need to migrate.");
      return;
    }

    if (currentVersion < 0 || currentVersion > SyncConfiguration.CURRENT_VERSION) {
    ...
    Logger.warn(LOG_TAG, "Migrated from version " + currentVersion + " to " + desiredVersion + ".");
    versionPrefs.edit().putLong(SyncConfiguration.PREF_VERSION, desiredVersion).commit();
  }
}
